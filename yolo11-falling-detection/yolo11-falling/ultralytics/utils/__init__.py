...
_TQDM_BASE = tqdm_original if tqdm_original is not None else object

class TQDM(_TQDM_BASE):
    """
    A custom TQDM progress bar class that extends the original tqdm functionality.
    Fallback to object if tqdm is not available.
...
def yaml_save(file="data.yaml", data=None, header=""):
    ...
    if yaml is None:
        raise ImportError("yaml is required for yaml_save()")
...
def yaml_load(file="data.yaml", append_filename=False):
    ...
    with open(file, errors="ignore", encoding="utf-8") as f:
        ...
        if yaml is None:
            raise ImportError("yaml is required for yaml_load()")
...
def yaml_print(yaml_file: Union[str, Path, dict]) -> None:
    ...
    if yaml is None:
        raise ImportError("yaml is required for yaml_print()")
